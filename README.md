...
- The same publish workflow also builds `dist/packs/oauth-oidc-executable.gtpack` and publishes it to GHCR as `ghcr.io/<owner>/oauth-oidc-executable-pack` (tagged by commit SHA, plus `latest` on `master` and the Git tag name on tagged releases).
